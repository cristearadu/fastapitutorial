...
@app.get("/books/")
async def read_all_book_by_rating(book_rating: int = Query(gt=0, lt=6)):
    books_to_return = []
    for book in BOOKS:
...
async def read_all_books_by_publish_date(published_date: int = Query(gt=1999, le=datetime.now().year)):
    books_to_return = []
    for book in BOOKS:
